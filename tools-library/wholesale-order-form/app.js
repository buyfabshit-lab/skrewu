/* ============================================================
   Wholesale Order Form — multi-item shop drop-off intake
   Shops fill this out; each submission lands in a Supabase table
   with artwork uploaded to a storage bucket.

   Portable tool module. Config is external: config.json (same folder)
   or ?config=<url>, falling back to DEFAULT_CONFIG so it runs from disk.
   See README.md for the config schema and API surface.
   ============================================================ */
...
const DEFAULT_CONFIG = {
  branding: {
    ...
    productName: 'Shop Order Intake',
    tagline: 'Send it to the floor',
    colors: { accent: '#a32a1f', accent2: '#c43a2c', highlight: '#c4f135' },
  },
  backend: {
    supabaseUrl: 'https://qmztuagvxopahowexrum.supabase.co',
    supabaseAnonKey: 'sb_publishable_cbwgMdVv6XDxLp0WOBsM-w_irvs7BAh',
    table: 'shop_orders',
    artworkBucket: 'order-artwork',
  },
  orderRefPrefix: 'SU',
};
...
let sb = null;
let ARTWORK_BUCKET = DEFAULT_CONFIG.backend.artworkBucket;
let TABLE = DEFAULT_CONFIG.backend.table;
let REF_PREFIX = DEFAULT_CONFIG.orderRefPrefix;
...
async function loadConfig() {
  ...
  try {
    ...
    return {
      ...
      orderRefPrefix: raw.orderRefPrefix || DEFAULT_CONFIG.orderRefPrefix,
    };
  } catch (e) {
    console.info('[WholesaleForm] using built-in default config (' + e.message + ')');
    return DEFAULT_CONFIG;
  }
}
function applyBranding(cfg) {
  const b = cfg.branding;
  document.title = b.businessName + ' — ' + b.productName;
  const nameEl = document.getElementById('brandName');
  if (nameEl) nameEl.firstChild ? (nameEl.firstChild.textContent = b.businessName) : (nameEl.textContent = b.businessName);
  const subEl = document.getElementById('brandSub');
  if (subEl) subEl.textContent = 'Order Intake · ' + b.tagline;
  const tagEl = document.getElementById('brandTag');
  if (tagEl) tagEl.textContent = b.tagline;
  const c = b.colors || {};
  const root = document.documentElement.style;
  if (c.accent)    root.setProperty('--rust', c.accent);
  if (c.accent2)   root.setProperty('--rust-bright', c.accent2);
  if (c.highlight) root.setProperty('--acid', c.highlight);
}
...
async function boot() {
  ...
  ARTWORK_BUCKET = CONFIG.backend.artworkBucket;
  TABLE = CONFIG.backend.table;
  REF_PREFIX = CONFIG.orderRefPrefix;
  sb = supabase.createClient(CONFIG.backend.supabaseUrl, CONFIG.backend.supabaseAnonKey);
  addItem(); // start with one empty item card
}
boot();
